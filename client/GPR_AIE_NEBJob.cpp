...
std::vector<std::string> GPR_AIE_NEBJob::run(void)
{
    ...
    this->fCallsGPR += 1;
    // helper_functions::printGPRsurface(gpf, 0);
    while(!converged){
        ...
        if (status == -9 ){
            std::cout<<"\nErly stopping from outer loop";
            matvec = gpnebInit.getCurPath();
            gpf->retrainGPR(matvec);
            this->fCallsGPR += 1;
            // helper_functions::printGPRsurface(gpf, this->fCallsGPR);
            continue;
        }
        ...
        while(mustUpdate){
            ...
            status = gpnebTwo.compute(prevpath);
            this->fCallsNEB += 1;
        if (status == -9 ){
            std::cout<<"\nEARLY STOPPING IN RELAXATION LOOP\n";
